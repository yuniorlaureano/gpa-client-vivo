...
@Component({
  ...
  templateUrl: './unit-list-table.component.html',
})
export class UnitListTableComponent implements OnInit, OnDestroy {
  @Output() onDelete = new EventEmitter<UnitModel>();
  @Output() onEdit = new EventEmitter<UnitModel>();
    ...
    const sub = this.pageOptionsSubject
      .pipe(
        switchMap((search) => {
          ...
          return this.unitService.getUnits(searchModel);
        })
      )
      .subscribe({
        next: (data) => {
